 ...
 * backend `GET /api/v1/defaults` endpoint.
 *
...
const _defaults = ref<SwarmDefaults>(_loadFromStorage() ?? _hardcodedDefaults());
let _fetchPromise: Promise<void> | null = null;

async function _fetchDefaults(baseUrl: string): Promise<void> {
  const url = `${baseUrl.replace(/\/$/, "")}/api/v1/defaults`;
  const resp = await fetch(url, { cache: "no-store" });
  if (!resp.ok) throw new Error(`GET /api/v1/defaults returned ${resp.status}`);
  ...
  if (
    ...
    throw new Error("Malformed /api/v1/defaults response");
  }
  _defaults.value = data;
  _saveToStorage(data);
}
...
export function useSwarmDefaults(baseUrl = "") {
  if (_fetchPromise === null) {
    _fetchPromise = _fetchDefaults(baseUrl).catch((err) => {
      ...
      console.warn("useSwarmDefaults: failed to fetch /api/v1/defaults —", err);
