...
async function loadEntries(): Promise<void> {
  ...
  try {
    ...
    const data = (await resp.json()) as { entries?: MemoryEntry[]; error?: string };
    if (data.error) {
...
async function deleteEntry(entry: MemoryEntry): Promise<void> {
  ...
  if (idx < 0) return;
  const resp = await fetch(apiUrl(`/v1/memory/notes/${idx}`), { method: "DELETE" });
  if (!resp.ok) {
...
async function consolidate(): Promise<void> {
  ...
  try {
    const resp = await fetch(apiUrl("/v1/memory/consolidate"), { method: "POST" });
    if (!resp.ok) {
      consolidateStatus.value = `Error HTTP ${resp.status}`;
      return;
    }
    ...
    consolidateStatus.value = e instanceof Error ? e.message : String(e);
  } finally {
