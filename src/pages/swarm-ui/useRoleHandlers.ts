...
export interface RoleHandlers {
  ...
  onRolePromptTextInput: (roleId: string, val: string) => void;
...
export function useRoleHandlers(settings: SettingsApi): RoleHandlers {
  ...
  function onRolePromptTextInput(roleId: string, val: string): void {
    settings.rolesState.roleStates[roleId].promptText = val;
    settings.saveSettingsSoon();
  }
